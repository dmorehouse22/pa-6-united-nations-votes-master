...
```{r ggplot-unvotes}
unvotes |> 
  count(country, vote) |> #creates a summary table with the number of votes per country
  group_by(country) |> #groups by country
  mutate(total = sum(n)) |> #creates a new variable in the summary table, sum of the number of votes
  mutate(prop_vote = n/total) |> #creates an additional variable, proportion of total votes per country/total votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #filters specific countries in the dataset
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #creates figure & mapping, x-variable = country, y-variable = prop_vote, fill is what is being plotted
  geom_col(position = position_stack()) + #creates a stacked bar chart with different outcomes of a vote
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #making the figure look pretty
  theme_minimal() +  #adds a theme to the figure
  scale_fill_viridis_d(end = 0.8) + #changes color of figure, helps with color blind viewers
...
The code results a short description of the votes
...
> Variables `para` or `amen` couuld be useful in the graphic
...
> The information is disorganized, not very explanatory. 
...
> The number of rows & columns do NOT match, calling for a joining of the datasets
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #adds matching rcid data from issues into roll_calls
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #adds matching rcid values from roll_calls and adding to issues
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #combination of both datasets rcid values in their entirety, whether they match or not
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #joining the matching values of rcid from both datasets
```
...
roll_calls |> 
  ...
  mutate(issue = case_when(
    ...
    issue == "Palestinian conflict" ~ "Palestinian Conflict")) |>   # its creating a new variable called `issue`, renaming the issues 
  drop_na(issue) |> # drop NA's for values not defined in the previous line of code
  mutate(year = lubridate::year(date)) |> # create a column `year` that contains the year value
  count(year, issue) |> # counts the number of issues within each year
  ggplot(aes(x = year, y = n, group = issue)) + # creates the figure and the mapping, groups by issue
  geom_line(aes(color = issue)) + # creates a line graph, adds color to distinguish the issues
  labs(x = "Year", y = "Number of Votes",
...
> Arms control had the highest peak of any voting event by the UN. The trend of voting increased over the years. 
